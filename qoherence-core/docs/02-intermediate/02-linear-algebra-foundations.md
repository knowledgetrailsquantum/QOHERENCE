...
**A quick analogy for why an infinite-feeling space isn't a problem: Hilbert's Hotel.** Mathematician David Hilbert (the same name behind "Hilbert space") once illustrated infinite sets with a thought experiment: imagine a hotel with infinitely many rooms, all full. A new guest arrives — no problem, just move the guest in room 1 to room 2, room 2's guest to room 3, and so on forever, freeing up room 1. A space that can always make room for more, without ever running out, is exactly the flavor of "large but well-behaved" space that a Hilbert space is — it's why `n` qubits can always be extended to `n+1` qubits without breaking any of the math from this chapter, no matter how large `n` already is.
